...
let walletConfig = config.get('proton').config;
let paytxfee = config.get('proton').paytxfee;
const proton = new bitcoindRpc(walletConfig);
...
function doBalance(message, tipper) {
  proton.getBalance(tipper, 1, function(err, balance) {
    if (err) {
      message.reply('Error getting Proton (PROTON) balance.').then(m => {
      ...
      message.channel.send({
        embeds: [
          {
            title:
              '**:bank::money_with_wings::moneybag:Proton (PROTON) Balance!:moneybag::money_with_wings::bank:**',
            color: 1363892,
            fields: [
              {
                name: '__User__',
    ...
    if (err) {
      ...
      message.channel.send({
        embeds: [
          {
            title:
              '**:bank::card_index::moneybag:Proton (PROTON) Address!:moneybag::card_index::bank:**',
            color: 1363892,
            fields: [
              {
                name: '__User__',
      ...
      if (Number(amount) + Number(paytxfee) > Number(balance)) {
        message.channel.send(
          'Please leave atleast ' +
            paytxfee +
            ' Proton (PROTON) for transaction fees!'
        );
        return;
      }
        ...
        if (err) {
          ...
          message.channel.send({
            embeds: [
              {
                title:
                  '**:outbox_tray::money_with_wings::moneybag:Proton (PROTON) Transaction Completed!:moneybag::money_with_wings::outbox_tray:**',
                color: 1363892,
                fields: [
                  {
                    name: '__Sender__',
...
function sendPROTON(bot, message, tipper, recipient, amount, privacyFlag) {
  getAddress(recipient.toString(), function(err, address) {
    if (err) {
      ...
      proton.sendFrom(tipper, address, Number(amount), 1, null, null, function(
        err,
        txId
      ) {
        if (err) {
          ...
          if (privacyFlag) {
            ...
            userProfile.user.send({
              embeds: [
                {
                  title:
                    '**:money_with_wings::moneybag:Proton (PROTON) Transaction Completed!:moneybag::money_with_wings:**',
                  color: 1363892,
                  fields: [
                    {
                      name: '__Sender__',
...
function addyLink(address) {
  return 'https://explorer.protoncoin.io/address/' + address;
}
