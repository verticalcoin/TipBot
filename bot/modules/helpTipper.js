...
// ⚡ Bolt Performance Optimization: Pre-calculate and batch help messages
// This avoids string concatenation on every command invocation and reduces
// Discord API requests from 7 to 2 by grouping messages within the 2000 char limit.
const helpTexts = [
...
for (const text of helpTexts) {
  if (currentMsg.length + text.length > 1900) {
    batchedMessages.push(currentMsg);
    currentMsg = text;
  } else {
