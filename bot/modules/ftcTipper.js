...
const ftc = new bitcoindRpc(walletConfig);
...
function doBalance(message, tipper) {
  ftc.getBalance(tipper, 1, function(err, balance) {
    if (err) {
      message.reply('Error getting Feathercoin (FTC) balance.').then(m => {
      ...
      message.channel.send({
        embeds: [
          {
            title:
              '**:bank::money_with_wings::moneybag:Feathercoin (FTC) Balance!:moneybag::money_with_wings::bank:**',
            color: 1363892,
            fields: [
              {
                name: '__User__',
...
function doDeposit(message, tipper) {
  getAddress(tipper, function(err, address) {
    if (err) {
      message
        .reply('Error getting your Feathercoin (FTC) deposit address.')
        .then(m => {
          setTimeout(() => m.delete().catch(() => {}), 10000);
        });
    } else {
      message.channel.send({
        embeds: [
          {
            title:
              '**:bank::card_index::moneybag:Feathercoin (FTC) Address!:moneybag::card_index::bank:**',
            color: 1363892,
            fields: [
              {
                name: '__User__',
...
function doWithdraw(message, tipper, words, helpmsg) {
  ...
  if (amount === null) {
    message
      .reply("I don't know how to withdraw that much Feathercoin (FTC)...")
      .then(m => {
        setTimeout(() => m.delete().catch(() => {}), 10000);
      });
    return;
      ...
      if (Number(amount) + Number(paytxfee) > Number(balance)) {
        message.channel.send(
          'Please leave atleast ' +
            paytxfee +
            ' Feathercoin (FTC) for transaction fees!'
        );
        return;
      }
        ...
        if (err) {
          ...
          message.channel.send({
            embeds: [
              {
                title:
                  '**:outbox_tray::money_with_wings::moneybag:Feathercoin (FTC) Transaction Completed!:moneybag::money_with_wings::outbox_tray:**',
                color: 1363892,
                fields: [
                  {
                    name: '__Sender__',
...
function doTip(bot, message, tipper, words, helpmsg) {
  ...
  if (amount === null) {
    message
      .reply("I don't know how to tip that much Feathercoin (FTC)...")
      .then(m => {
        setTimeout(() => m.delete().catch(() => {}), 10000);
      });
    return;
...
function sendFTC(bot, message, tipper, recipient, amount, privacyFlag) {
  getAddress(recipient.toString(), function(err, address) {
    if (err) {
      ...
      ftc.sendFrom(tipper, address, Number(amount), 1, null, null, function(
        err,
        txId
      ) {
        if (err) {
          ...
          if (privacyFlag) {
            ...
            userProfile.user.send({
              embeds: [
                {
                  title:
                    '**:money_with_wings::moneybag:Feathercoin (FTC) Transaction Completed!:moneybag::money_with_wings:**',
                  color: 1363892,
                  fields: [
                    {
                      name: '__Sender__',
            ...
            if (message.content.startsWith('!tipftc private ')) {
